...
def get_skills_for_direction(direction: str):
    ...
    # В зависимости от направления выбираем листы
    skills = []
    if direction == 'Product':
        # Hard + Soft
        for cat in ['Hard', 'Soft']:
            if cat in matrix:
                for skill_data in matrix[cat]['skills'].values():
                    skills.append(Skill(name=skill_data['name'], group=skill_data['group'], levels=skill_data['levels']))
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if role == 'employee':
        ...
        # Показать список дизайнеров своего направления
...
async def show_lead_dashboard(message: types.Message, direction: str):
    """Показывает лиду список дизайнеров его направления с результатами."""
    ...
    if pd.io.common.file_exists(RESULTS_FILE):
        ...
        # Можно добавить инлайн-кнопки для просмотра деталей каждого дизайнера
...
async def show_admin_dashboard(message: types.Message):
    """Показывает администратору общую статистику."""
    if pd.io.common.file_exists(RESULTS_FILE):
        df_results = pd.read_csv(RESULTS_FILE)
        if df_results.empty:
            await message.answer("Нет данных о результатах.")
        ...
        # Также можно показать распределение по направлениям
        ...
        # Генерируем гистограмму (график) и отправляем
        # (реализуйте отдельно)
    else:
        await message.answer("Нет данных о результатах.")
...
async def ask_skill(message: types.Message, state: FSMContext):
    ...
    skill = skills[index]
    # Создаём клавиатуру с вариантами 0-4
    ...
    await message.answer(f"Вопрос {index+1}/{len(skills)}\nНавык: *{skill.name}*\nГруппа: {skill.group}\n\nОцените свой уровень по шкале:\n{scale_desc}",
...
async def finish_survey(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"✅ Опрос завершён!\n\nВаш уровень: *{level}*\nСредний балл: {avg_score:.2f}\n\nРекомендации:\n{rec_text}\n\nПлан развития:\n{plan_text}",
                         parse_mode="Markdown")
